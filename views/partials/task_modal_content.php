...
<?php if ($canAssign || $canUnassign || $canEditTask || $canDeleteTask): ?>
  <div class="mt-3 d-flex flex-wrap gap-2 align-items-end">
    <?php if ($canAssign): ?>
      <form class="ajax-form flex-grow-1" data-reload="task-modal" data-task-id="<?= $task->getId() ?>" method="POST" action="<?= Config::getBaseUrl() ?>controllers/task_controller.php">
    ...
    <div class="d-flex gap-2">
      ...
      <?php if ($canEditTask): ?>
        <button class="btn btn-warning edit-task-btn" data-task-id="<?= $task->getId() ?>">Edit Task</button>
      <?php endif; ?>
...
<?php if ($canComment): ?>
  <div class="card mb-3">
    <div class="card-body">
      <form id="add-comment-form-modal" class="ajax-form" data-reload="task-modal" data-task-id="<?= $task->getId() ?>" method="POST" action="<?= Config::getBaseUrl() ?>controllers/task_controller.php">
