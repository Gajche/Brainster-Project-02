...
class Task
{
  ...
  public static function create($data, $userId)
  {
    ...
    $db = Database::getInstance();
    $stmt = $db->prepare('INSERT INTO tasks (project_id, title, description, created_by) VALUES (?, ?, ?, ?)');  // Added created_by
    if ($stmt->execute([$data['project_id'], $data['title'], $data['description'], $userId])) {  // Pass $userId as creator
